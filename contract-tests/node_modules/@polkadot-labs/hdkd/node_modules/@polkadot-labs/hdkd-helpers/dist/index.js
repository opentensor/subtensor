...
var mnemonicToEntropy = (mnemonic, wordlistStr = BIP39_EN_WORDLIST) => {
  ...
  const entropyBytes = bits.slice(0, dividerIndex).match(/(.{1,8})/g)?.map((bin) => Number.parseInt(bin, 2));
...
var entropyToMnemonic = (entropy, wordlistStr = BIP39_EN_WORDLIST) => {
  ...
  return (entropyBits + deriveChecksumBits(entropy)).match(/(.{1,11})/g).map((binary) => wordlist[Number.parseInt(binary, 2)]).join(" ");
};
...
import { Bytes, str as str2, Tuple } from "scale-ts";
var blake2b256 = (msg) => blake2b(ensureBytes("msg", msg), { dkLen: 32 });
var blake2b512 = (msg) => blake2b(ensureBytes("msg", msg), { dkLen: 64 });
...
var derivationCodec = /* @__PURE__ */ Tuple(str2, Bytes(32), Bytes(32));
...
import { getPublicKey as getPublicKey2, HDKD, secretFromSeed } from "@scure/sr25519";
