...
// src/internal/share-latest.ts
var shareLatest = (source$, defaultValue, teardown = noop) => {
  let subject;
  let subscription;
  let refCount = 0;
  ...
  result.getComplete$ = () => new Observable((observer) => {
    if (refCount === 0) {
...
function connectFactoryObservable(getObservable, defaultValue) {
  ...
  const getSharedObservables$ = (input) => {
    ...
    const sharedObservable$ = share_latest_default(new Observable2((observer) => getObservable(...input).subscribe(observer)), getDefaultValue(...input), () => {
      cache.delete(keys);
    });
...
//# sourceMappingURL=rxjs-state.es2019.mjs.map
