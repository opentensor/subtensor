...
describe("bridge token contract deployment", () => {
  ...
  it("Can add stake V2", async () => {
    ...
    const stakeBefore = await api.query.SubtensorModule.AlphaV2.getValue(convertPublicKeyToSs58(hotkey.publicKey), convertH160ToSS58(wallet.address), netuid)
    ...
    const stakeAfter = await api.query.SubtensorModule.AlphaV2.getValue(convertPublicKeyToSs58(hotkey.publicKey), convertH160ToSS58(wallet.address), netuid)
  ...
  it("Can deploy alpha pool smart contract", async () => {
    ...
    const stakeBeforeDeposit = await api.query.SubtensorModule.AlphaV2.getValue(convertPublicKeyToSs58(hotkey.publicKey), convertH160ToSS58(wallet.address), netuid)
    ...
    const stakeAftereDeposit = await api.query.SubtensorModule.AlphaV2.getValue(convertPublicKeyToSs58(hotkey.publicKey), convertH160ToSS58(wallet.address), netuid)
    assert.ok(stakeAftereDeposit < stakeBeforeDeposit)

    // check the contract stake
    const ContractStake = await api.query.SubtensorModule.AlphaV2.getValue(convertPublicKeyToSs58(hotkey.publicKey), convertH160ToSS58(contractAddress), netuid)
