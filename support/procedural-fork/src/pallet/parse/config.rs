...
impl TryFrom<&syn::TraitItemType> for ConstMetadataDef {
    ...
    fn try_from(trait_ty: &syn::TraitItemType) -> Result<Self, Self::Error> {
        let err = |span, msg| {
            syn::Error::new(
                span,
                format!("Invalid usage of `#[pallet::constant]`: {}", msg),
            )
        };
...
fn check_event_type(
    ...
    let syn::TraitItem::Type(type_) = trait_item else {
        return Ok(false);
    };
...
mod tests {
    ...
    fn has_expected_system_config_works_with_frame_full_path() {
        ...
        let path = syn::parse2::<syn::Path>(quote::quote!(
            polkadot_sdk_frame::deps::frame_system::Config
        ))
        .unwrap();
        assert!(has_expected_system_config(path, &frame_system));
