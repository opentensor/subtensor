...
impl Def {
    ...
    fn resolve_manual_task_enum(
        ...
        let (None, Some(tasks)) = (&task_enum, &tasks) else {
            return Ok(());
        };
        let syn::Type::Path(type_path) = &*tasks.item_impl.self_ty else {
            return Ok(());
        };
        ...
        for item in items {
            ...
            if item_enum.ident == seg.ident {
                result = Some(syn::parse2::<tasks::TaskEnumDef>(
    ...
    pub fn resolve_manual_tasks_impl(
        ...
        for item in items {
            ...
            let Some(trait_last_seg) = path.segments.last() else {
                continue;
            };
            let syn::Type::Path(target_path) = &*item_impl.self_ty else {
                continue;
            };
...
impl syn::parse::Parse for PalletAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if lookahead.peek(keyword::config) {
            ...
            Ok(PalletAttr::Tasks(
                content.parse::<keyword::tasks_experimental>()?.span(),
            ))
        } else if lookahead.peek(keyword::task_enum) {
            Ok(PalletAttr::RuntimeTask(
                content.parse::<keyword::task_enum>()?.span(),
            ))
        } else if lookahead.peek(keyword::task_condition) {
            Ok(PalletAttr::TaskCondition(
                content.parse::<keyword::task_condition>()?.span(),
            ))
        } else if lookahead.peek(keyword::task_index) {
            Ok(PalletAttr::TaskIndex(
                content.parse::<keyword::task_index>()?.span(),
            ))
        } else if lookahead.peek(keyword::task_list) {
            Ok(PalletAttr::TaskList(
                content.parse::<keyword::task_list>()?.span(),
            ))
        } else if lookahead.peek(keyword::error) {
            Ok(PalletAttr::Error(content.parse::<keyword::error>()?.span()))
        } else if lookahead.peek(keyword::event) {
            Ok(PalletAttr::RuntimeEvent(
                content.parse::<keyword::event>()?.span(),
            ))
        } else if lookahead.peek(keyword::origin) {
            Ok(PalletAttr::RuntimeOrigin(
            ...
            Ok(PalletAttr::Composite(
                content.parse::<keyword::composite_enum>()?.span(),
            ))
        } else {
            Err(lookahead.error())
        }
