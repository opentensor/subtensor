...
impl syn::parse::Parse for PalletStorageAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if lookahead.peek(keyword::getter) {
            ...
            Ok(Self::Getter(
                generate_content.parse::<syn::Ident>()?,
                attr_span,
            ))
...
impl StorageGenerics {
    ...
    fn metadata(&self) -> syn::Result<Metadata> {
        let res = match self.clone() {
            Self::DoubleMap {
                value, key1, key2, ..
            } => Metadata::DoubleMap { value, key1, key2 },
            ...
            Self::CountedNMap { keygen, value, .. } => Metadata::CountedNMap {
                keys: collect_keys(&keygen)?,
                keygen,
                value,
            },
...
impl StorageDef {
    ...
    pub fn try_from(
        ...
        let instances = vec![helper::check_type_def_gen(
            &item.generics,
            item.ident.span(),
        )?];
            ...
            .map(|query_kind| {
                ...
                let result_query = match query_kind {
                    ...
                    Type::Path(TypePath {
                        path: Path { segments, .. },
                        ..
                    }) if segments.last().map_or(false, |s| s.ident == "ResultQuery") => segments
                        .last()
