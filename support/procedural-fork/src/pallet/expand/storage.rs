...
pub fn process_generics(def: &mut Def) -> syn::Result<Vec<ResultOnEmptyStructMetadata>> {
    ...
    for storage_def in def.storages.iter_mut() {
        ...
        if let Some(named_generics) = storage_def.named_generics.clone() {
            args.args.clear();
            args.args
                .push(syn::parse_quote!( #prefix_ident<#type_use_gen> ));
            ...
            {
                args.args
                    .push(syn::GenericArgument::Type(syn::parse_quote!(#error_path)))
            }
            ...
            if on_empty_idx >= args.args.len()
                && matches!(
                    storage_def.query_kind.as_ref(),
                    Some(QueryKind::ResultQuery(_, _))
                )
            {
...
fn augment_final_docs(def: &mut Def) {
    ...
    def.storages
        .iter_mut()
        .for_each(|storage| match &storage.metadata {
            ...
            Some(PathSegment {
                ...
                let (has_config, has_instance) =
                    args.args
                        ...
                        .fold(
                            (false, false),
                            |(has_config, has_instance), arg| match arg {
