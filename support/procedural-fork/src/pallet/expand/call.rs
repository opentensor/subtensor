...
pub fn expand_call(def: &mut Def) -> proc_macro2::TokenStream {
    ...
    let maybe_allow_attrs = methods
        ...
        .map(|method| {
            method
                ...
                .map_or(proc_macro2::TokenStream::new(), |attr| {
    ...
    let feeless_check = methods
        .iter()
        .map(|method| &method.feeless_check)
        .collect::<Vec<_>>();
    let feeless_check_result =
        feeless_check
            .iter()
            .zip(args_name.iter())
            .map(|(feeless_check, arg_name)| {
