...
pub fn expand_tasks_impl(def: &mut Def) -> TokenStream2 {
    let Some(tasks) = &mut def.tasks else {
        return quote!();
    };
    let ExpandedTasksDef {
        task_item_impl,
        task_trait_impl,
    } = parse_quote!(#tasks);
    quote! {
...
pub fn expand_task_enum(def: &mut Def) -> TokenStream2 {
    let Some(task_enum) = &mut def.task_enum else {
        return quote!();
    };
    let ExpandedTaskEnum {
        item_enum,
        debug_impl,
    } = parse_quote!(#task_enum);
    quote! {
        #item_enum
        #debug_impl
    }
