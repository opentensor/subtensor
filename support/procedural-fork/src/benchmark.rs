...
impl BenchmarkCallDef {
    ...
    fn attr_span(&self) -> Span {
        match self {
            BenchmarkCallDef::ExtrinsicCall {
                origin: _,
                expr_call: _,
                attr_span,
            } => *attr_span,
            BenchmarkCallDef::Block {
                block: _,
                attr_span,
            } => *attr_span,
        }
...
fn ensure_valid_return_type(item_fn: &ItemFn) -> Result<()> {
    if let ReturnType::Type(_, typ) = &item_fn.sig.output {
        ...
        let Type::Tuple(tup) = res.unit else {
            return non_unit(res.unit.span());
        };
...
fn parse_params(item_fn: &ItemFn) -> Result<Vec<ParamDef>> {
    ...
    for arg in &item_fn.sig.inputs {
        ...
        let Some(name_char) = name.chars().next() else {
            return invalid_param_name();
        };
        ...
        let Ok(args) = syn::parse::<RangeArgs>(args) else {
            return invalid_param(typ.span());
        };
...
impl BenchmarkDef {
    ...
    pub fn from(item_fn: &ItemFn) -> Result<BenchmarkDef> {
        ...
        let (verify_stmts, last_stmt) = match item_fn.sig.output {
            ...
            {
                (
                    Vec::from(&item_fn.block.stmts[(i + 1)..item_fn.block.stmts.len()]),
                    None,
                )
            }
            ReturnType::Type(_, _) => {
...
pub fn benchmarks(
    ...
    let where_clause = match syn::parse::<Nothing>(attrs.clone()) {
        Ok(_) => quote!(),
        Err(_) => syn::parse::<WhereClause>(attrs)?
            .predicates
            .to_token_stream(),
    };
    ...
    let (_brace, mut content) = module
        .content
        .ok_or(syn::Error::new(mod_span, "Module cannot be empty!"))?;
    ...
    let benchmark_fn_metas = content.iter_mut().filter_map(|stmt| {
        ...
        let benchmark_attr = func
            .attrs
            .iter()
            .find(|attr| attr.path().is_ident(keywords::BENCHMARK_TOKEN))?;
    ...
    let extra_benchmark_names_str: Vec<String> = extra_benchmark_names
        .iter()
        .map(|n| n.to_string())
        .collect();
    let skip_meta_benchmark_names_str: Vec<String> = skip_meta_benchmark_names
        .iter()
        .map(|n| n.to_string())
        .collect();
