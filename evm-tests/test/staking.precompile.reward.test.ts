...
describe("Test neuron precompile reward", () => {
  ...
  before(async () => {
    ...
    await forceSetBalanceToSs58Address(
      api,
      convertPublicKeyToSs58(validator.publicKey),
    );
    await forceSetBalanceToSs58Address(
      api,
    ...
    await forceSetBalanceToSs58Address(
      api,
      convertPublicKeyToSs58(nominator.publicKey),
    );
  ...
  it("Staker receives rewards", async () => {
    ...
    const miner_alpha_before_emission = await api.query.SubtensorModule.Alpha
      .getValue(
        convertPublicKeyToSs58(miner.publicKey),
        convertPublicKeyToSs58(coldkey.publicKey),
        netuid,
    ...
    let index = 0;
    while (index < 60) {
      const pending = await api.query.SubtensorModule.PendingEmission.getValue(
        netuid,
      );
      if (pending > 0) {
