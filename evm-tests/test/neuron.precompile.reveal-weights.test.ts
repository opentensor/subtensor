...
import { Tuple, u16, u64, u8, Vec, VecFixed } from "@polkadot/types-codec";
...
describe("Test neuron precompile reveal weights", () => {
  ...
  before(async () => {
    ...
    await forceSetBalanceToSs58Address(
      api,
      convertPublicKeyToSs58(alice.publicKey),
    );
    await forceSetBalanceToSs58Address(
      api,
  ...
  it("EVM neuron commit weights via call precompile", async () => {
    ...
    const weightsCommit = await api.query.SubtensorModule.WeightCommits
      .getValue(subnetId, ss58Address);
  ...
  it("EVM neuron reveal weights via call precompile", async () => {
    ...
    const weights = await api.query.SubtensorModule.Weights.getValue(
      netuid,
      neuron_uid,
    );
