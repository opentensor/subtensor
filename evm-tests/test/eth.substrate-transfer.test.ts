...
import { Binary, FixedSizeBinary, TypedApi } from "polkadot-api";
import { generateRandomEthersWallet } from "../src/utils";
import {
  bigintToRao,
  compareEthBalanceWithTxFee,
  raoToEth,
  tao,
...
describe("Balance transfers between substrate and EVM", () => {
  ...
  before(async () => {
    ...
    await forceSetBalanceToSs58Address(
      api,
      convertPublicKeyToSs58(signer.publicKey),
    );
  ...
  it("Can transfer token from EVM to EVM", async () => {
    const senderBalance = await publicClient.getBalance({
      address: toViemAddress(wallet.address),
    });
    const receiverBalance = await publicClient.getBalance({
      address: toViemAddress(wallet2.address),
    });
    ...
    const senderBalanceAfterTransfer = await publicClient.getBalance({
      address: toViemAddress(wallet.address),
    });
    const receiverBalanceAfterTranser = await publicClient.getBalance({
      address: toViemAddress(wallet2.address),
    });

    assert.equal(
      senderBalanceAfterTransfer,
      senderBalance - transferBalance - txFee,
    );
    assert.equal(
      receiverBalance,
      receiverBalanceAfterTranser - transferBalance,
    );
  });
  ...
  it("Can transfer token from Substrate to EVM", async () => {
    ...
    assert.equal(
      receiverBalance,
      receiverBalanceAfterTranser - raoToEth(transferBalance),
    );
  });
  ...
  it("Can transfer token from EVM to Substrate", async () => {
    ...
    const receiverBalance = (await api.query.System.Account.getValue(
      convertPublicKeyToSs58(signer.publicKey),
    )).data.free;
    const transferBalance = raoToEth(tao(1));

    const tx = await contract.transfer(signer.publicKey, {
      value: transferBalance.toString(),
    });
    await tx.wait();
    ...
    const receiverBalanceAfterTranser =
  ...
  it("Transfer from EVM to substrate using evm::withdraw", async () => {
    ...
    const senderBalanceAfterWithdraw =
      (await api.query.System.Account.getValue(ss58Address)).data.free;
  ...
  it("Transfer from EVM to substrate using evm::call", async () => {
    ...
    const receiverBalanceAfterCall = await publicClient.getBalance({
      address: toViemAddress(wallet.address),
    });
  ...
  it("Forward value in smart contract", async () => {
    ...
    const callerBalance = await publicClient.getBalance({
      address: toViemAddress(wallet.address),
    });

    const contractForCall = new ethers.Contract(
    ...
    const callerBalanceAfterWithdraw = await publicClient.getBalance({
      address: toViemAddress(wallet.address),
    });

    compareEthBalanceWithTxFee(
      callerBalanceAfterWithdraw,
      callerBalance + raoToEth(tao(1)),
    );
    assert.equal(
      contractBalance,
      contractBalanceAfterWithdraw + raoToEth(tao(1)),
    );
  });
  ...
  it("Transfer full balance", async () => {
    ...
    const receiverBalanceAfterTransfer = await publicClient.getBalance({
      address: toViemAddress(wallet2.address),
    });
  ...
  it("Low max_fee_per_gas gets transaction rejected", async () => {
    try {
      ...
      if (error instanceof Error) {
        console.log(error.toString());
        assert.equal(
          error.toString().includes("gas price less than block base fee"),
          true,
        );
      }
    }
  ...
  it("max_fee_per_gas lower than max_priority_fee_per_gas gets transaction rejected", async () => {
    try {
      ...
      if (error instanceof Error) {
        assert.equal(
          error.toString().includes("priorityFee cannot be more than maxFee"),
          true,
        );
      }
    }
...
async function transferAndGetFee(
  ...
  const ethBalanceBefore = await client.getBalance({
    address: toViemAddress(wallet.address),
  });
  // Send TAO
  const tx = {
  ...
  // Check balances
  const ethBalanceAfter = await client.getBalance({
    address: toViemAddress(wallet.address),
  });
