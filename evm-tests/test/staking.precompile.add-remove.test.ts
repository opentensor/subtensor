...
describe("Test neuron precompile add remove stake", () => {
  ...
  before(async () => {
    ...
    await forceSetBalanceToSs58Address(
      api,
      convertPublicKeyToSs58(proxy.publicKey),
    );
  ...
  it("Can add stake", async () => {
    ...
    const tx = await contract.addStake(hotkey.publicKey, netuid, {
      value: stakeBalance.toString(),
    });
    await tx.wait();

    const stakeFromContract = BigInt(
      await contract.getStake(
        hotkey.publicKey,
  ...
  it("Can add/remove proxy", async () => {
    ...
    const proxiesAfterRemove = await api.query.Proxy.Proxies.getValue(
      ss58Address,
    );
