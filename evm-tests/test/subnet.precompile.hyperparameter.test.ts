...
import { ISUBNET_ADDRESS, ISubnetABI } from "../src/contracts/subnet";
...
describe("Test the Subnet precompile contract", () => {
  ...
  it("Can register network without identity info", async () => {
    const totalNetwork = await api.query.SubtensorModule.TotalNetworks
      .getValue();
    ...
    const totalNetworkAfterAdd = await api.query.SubtensorModule.TotalNetworks
      .getValue();
  ...
  it("Can set servingRateLimit parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.ServingRateLimit
      .getValue(netuid);
  ...
  it("Can set maxDifficulty parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.MaxDifficulty.getValue(
      netuid,
    );
  ...
  it("Can set weightsVersionKey parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.WeightsVersionKey
      .getValue(netuid);
  ...
  it("Can set adjustmentAlpha parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.AdjustmentAlpha.getValue(
      netuid,
    );
  ...
  it("Can set maxWeightLimit parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.MaxWeightsLimit.getValue(
      netuid,
    );
  ...
  it("Can set immunityPeriod parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.ImmunityPeriod.getValue(
      netuid,
    );
  ...
  it("Can set minAllowedWeights parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.MinAllowedWeights
      .getValue(netuid);
  ...
  it("Can set activityCutoff parameter", async () => {
    ...
    const netuid = totalNetwork - 1;
    const newValue =
      await api.query.SubtensorModule.MinActivityCutoff.getValue() + 1;
    const tx = await contract.setActivityCutoff(netuid, newValue);
    await tx.wait();

    let onchainValue = await api.query.SubtensorModule.ActivityCutoff.getValue(
      netuid,
    );
  ...
  it("Can set networkRegistrationAllowed parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule
      .NetworkRegistrationAllowed.getValue(netuid);
  ...
  it("Can set networkPowRegistrationAllowed parameter", async () => {
    ...
    const newValue = true;
    const tx = await contract.setNetworkPowRegistrationAllowed(
      netuid,
      newValue,
    );
    await tx.wait();
    ...
    let onchainValue = await api.query.SubtensorModule
      .NetworkPowRegistrationAllowed.getValue(netuid);
  ...
  it("Can set bondsMovingAverage parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.BondsMovingAverage
      .getValue(netuid);
  ...
  it("Can set commitRevealWeightsEnabled parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule
      .CommitRevealWeightsEnabled.getValue(netuid);
  ...
  it("Can set liquidAlphaEnabled parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.LiquidAlphaOn.getValue(
      netuid,
    );
  ...
  it("Can set alphaValues parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.AlphaValues.getValue(
      netuid,
    );
  ...
  it("Can set commitRevealWeightsInterval parameter", async () => {
    ...
    let onchainValue = await api.query.SubtensorModule.RevealPeriodEpochs
      .getValue(netuid);
