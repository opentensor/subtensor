...
export async function getNonceChangePromise(
  ...
  return new Promise<void>((resolve, reject) => {
    const subscription = api.query.System.Account.watchValue(ss58Address)
      .subscribe({
        next(value) {
...
export function convertPublicKeyToMultiAddress(
  publicKey: Uint8Array,
  ss58Format: number = SS58_PREFIX,
): MultiAddress {
...
export async function getTransactionWatchPromise(
  tx: Transaction<{}, string, string, void>,
  signer: PolkadotSigner,
) {
  return new Promise<void>((resolve, reject) => {
...
export async function waitForNonceChange(
  api: TypedApi<typeof devnet>,
  ss58Address: string,
) {
