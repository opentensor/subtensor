...
import { TxCallData, TypedApi } from "polkadot-api";
...
export async function addNewSubnetwork(
  ...
  if (rateLimit !== BigInt(0)) {
    const internalCall = api.tx.AdminUtils.sudo_set_network_rate_limit({
      rate_limit: BigInt(0),
    });
  ...
  assert.equal(
    totalNetworks + 1,
    await api.query.SubtensorModule.TotalNetworks.getValue(),
  );
...
export async function setCommitRevealWeightsEnabled(
  api: TypedApi<typeof devnet>,
  netuid: number,
  enabled: boolean,
) {
  const value = await api.query.SubtensorModule.CommitRevealWeightsEnabled
    .getValue(netuid);
  if (value === enabled) {
    return;
  }
  ...
  const alice = getAliceSigner();
  const internalCall = api.tx.AdminUtils.sudo_set_commit_reveal_weights_enabled(
  ...
  assert.equal(
    enabled,
    await api.query.SubtensorModule.CommitRevealWeightsEnabled.getValue(netuid),
  );
}

export async function setWeightsSetRateLimit(
  api: TypedApi<typeof devnet>,
  netuid: number,
  rateLimit: bigint,
) {
  const value = await api.query.SubtensorModule.WeightsSetRateLimit.getValue(
    netuid,
  );
  if (value === rateLimit) {
    return;
  ...
  const internalCall = api.tx.AdminUtils.sudo_set_weights_set_rate_limit({
    netuid: netuid,
    weights_set_rate_limit: rateLimit,
  });
  const tx = api.tx.Sudo.sudo({ call: internalCall.decodedCall });
  ...
  assert.equal(
    rateLimit,
    await api.query.SubtensorModule.WeightsSetRateLimit.getValue(netuid),
  );
}
...
export async function setTxRateLimit(
  ...
  const internalCall = api.tx.AdminUtils.sudo_set_tx_rate_limit({
  ...
  assert.equal(
    txRateLimit,
    await api.query.SubtensorModule.TxRateLimit.getValue(),
  );
}

export async function setMaxAllowedValidators(
  api: TypedApi<typeof devnet>,
  netuid: number,
  maxAllowedValidators: number,
) {
  const value = await api.query.SubtensorModule.MaxAllowedValidators.getValue(
    netuid,
  );
  ...
  assert.equal(
    ...
    await api.query.SubtensorModule.MaxAllowedValidators.getValue(netuid),
  );
}

export async function setSubnetOwnerCut(
  api: TypedApi<typeof devnet>,
  subnetOwnerCut: number,
) {
  ...
  assert.equal(
    subnetOwnerCut,
    await api.query.SubtensorModule.SubnetOwnerCut.getValue(),
  );
}

export async function setActivityCutoff(
  api: TypedApi<typeof devnet>,
  netuid: number,
  ...
  assert.equal(
    activityCutoff,
    await api.query.SubtensorModule.ActivityCutoff.getValue(netuid),
  );
}

export async function setMaxAllowedUids(
  api: TypedApi<typeof devnet>,
  netuid: number,
  ...
  assert.equal(
    maxAllowedUids,
    await api.query.SubtensorModule.MaxAllowedUids.getValue(netuid),
  );
}
...
export async function startCall(
  ...
  const duration = Number(
    await api.constants.SubtensorModule.DurationOfStartCall,
  );
