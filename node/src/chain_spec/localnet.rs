    ...
    serde_json::json!({
        ...
        "staking": {
            ...
            "stakers": initial_authorities
                .iter()
                .map(|x| (x.stash().clone(), x.controller().clone(), STAKE, StakerStatus::<AccountId>::Validator))
                .collect::<Vec<_>>(),
        ...
        "grandpa": {
            "authorities": initial_authorities.iter().map(|x| (x.stash().clone(), 1u64)).collect::<Vec<_>>(),
        },
        "sudo": {
