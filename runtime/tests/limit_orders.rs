...
/// Documents the MEVShield usage pattern: an order with `relayer: None` can be
/// submitted by any account on-chain, not just a pinned relayer.
///
/// Alice signs a LimitBuy order with `relayer: None`.  Dave — an account with no
/// relationship to the order — submits the `execute_orders` transaction.
///
/// On-chain there is no relayer restriction, so the call succeeds and the order
/// is stored as `Fulfilled`.  MEVShield protection operates purely at the mempool
/// level: validators running MEVShield refuse to propagate or include
/// `execute_orders` transactions that are not signed by an authorised relayer
/// account, but the pallet itself imposes no such constraint.
#[test]
fn execute_orders_no_relayer_any_account_can_relay() {
    new_test_ext().execute_with(|| {
        let netuid = NetUid::from(1u16);
        setup_subnet(netuid);
        ...
        // Fund Alice so the buy can debit her balance.
        fund_account(&alice_id);

        // Associate Alice (coldkey) with Bob (hotkey) so staking goes through Bob.
        let _ = SubtensorModule::create_account_if_non_existent(&alice_id, &bob_id);

        // A current timestamp is required; the order carries expiry = u64::MAX so
        // it will never be considered expired regardless of this value.
        pallet_timestamp::Now::<Runtime>::put(100_000u64);

        // Build the order via `make_signed_order_inner` with an explicit
        // `relayer: None` so the intent — no pinned relayer — is visible in the
        // test body rather than hidden inside a convenience wrapper.
        let signed = make_signed_order_inner(
            alice,
            bob_id.clone(),
            netuid,
            OrderParams {
                ...
                limit_price: u64::MAX, // price ceiling always satisfied
                expiry: u64::MAX,      // never expires
                fee_rate: Perbill::zero(),
                fee_recipient: charlie_id.clone(),
                relayer: None, // no pinned relayer — any account may submit
                max_slippage: None,
                partial_fills_enabled: false,
            },
        );
        ...
        // Dave submits the transaction — he is not Alice, Bob, or Charlie and has
        // no special relationship to the order.  The call must succeed because
        // `relayer: None` imposes no restriction on the origin.
        assert_ok!(LimitOrders::execute_orders(
            RuntimeOrigin::signed(dave_id),
            orders,
        ));

        // The order must be written to storage as Fulfilled.
        assert_eq!(
            Orders::<Runtime>::get(id),
            Some(OrderStatus::Fulfilled),
            "order signed by Alice with relayer=None should be fulfilled when Dave submits it"
        );
    });
}

/// Documents MEVShield usage with an explicit `relayer` field.
///
/// MEVShield is an encrypted mempool: the original submitter's origin is
/// preserved when the transaction is decrypted and included.  This means the
/// relayer does NOT need to be a MEVShield validator — it can be any account.
/// The user pins their order to a specific relayer account; that account
/// encrypts its `execute_orders` call via MEVShield; when included, the origin
/// is still that account and the pallet's `relayer` check passes.
///
/// Simulation: `RuntimeOrigin::signed(charlie_id)` is identical to what
/// MEVShield would deliver on-chain for a tx submitted by Charlie.
#[test]
fn execute_orders_mevshield_with_pinned_relayer() {
    new_test_ext().execute_with(|| {
        let netuid = NetUid::from(1u16);
        setup_subnet(netuid);
        ...
        // Alice pins the order to Charlie — a specific account, not a validator.
        // Charlie will submit via MEVShield; the origin is preserved as Charlie.
        let signed = make_signed_order_inner(
            alice,
            bob_id.clone(),
            netuid,
            OrderParams {
                ...
                fee_rate: Perbill::zero(),
                fee_recipient: charlie_id.clone(),
                relayer: Some(BoundedVec::try_from(vec![charlie_id.clone()]).unwrap()),
                max_slippage: None,
                partial_fills_enabled: false,
            },
        );
        ...
        // Dave attempts to relay — must be skipped because he is not in the relayer set.
        let dave_id = Sr25519Keyring::Dave.to_account_id();
        let orders = make_order_batch(vec![signed.clone()]);
        assert_ok!(LimitOrders::execute_orders(
            RuntimeOrigin::signed(dave_id),
            orders,
        ));
        assert!(
            Orders::<Runtime>::get(id).is_none(),
            "order should be skipped when submitted by an account not in the relayer set"
        );

        // Charlie submits — simulates MEVShield decrypting Charlie's tx and
        // including it with Charlie's origin intact.  Must execute.
        let orders = make_order_batch(vec![signed]);
        assert_ok!(LimitOrders::execute_orders(
            RuntimeOrigin::signed(charlie_id),
            orders,
        ...
        assert_eq!(
            Orders::<Runtime>::get(id),
            Some(OrderStatus::Fulfilled),
            "order should be fulfilled when submitted by the pinned relayer via MEVShield"
        );
    });
}
