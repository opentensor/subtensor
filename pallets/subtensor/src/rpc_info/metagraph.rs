...
impl<T: Config> Pallet<T> {
    pub fn get_metagraph(netuid: u16) -> Option<Metagraph<T>> {
        ...
        Some(Metagraph {
            ...
            // Name and symbol
            name: Self::get_name_for_subnet(netuid).into_iter().map(Compact).collect(), // Name
            symbol: Self::get_symbol_for_subnet(netuid).into_iter().map(Compact).collect(), // Symbol.           
            ...
            dividends: Dividends::<T>::get(netuid).into_iter().map(Compact::from).collect(), // Dividends per UID
            incentives: Incentive::<T>::get(netuid).into_iter().map(Compact::from).collect(), // Mining incentives per UID
