...
impl<T: Config> Pallet<T> 
{
    ...
    {
        let mut delegates: Vec<DelegateInfo<T>> = Vec::<DelegateInfo<T>>::new();
    ...
    pub fn get_delegated(delegatee_account_vec: Vec<u8>) -> Vec<(DelegateInfo<T>, Compact<u64>)> 
    {
        ...
        let delegatee: AccountIdOf<T> = T::AccountId::decode(
            &mut delegatee_account_vec.as_bytes_ref() 
        ).unwrap();
