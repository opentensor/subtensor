...
fn test_reveal_weights_validate() {
    ...
    new_test_ext(0).execute_with(|| {
        ...
        // Counter is 0 on a fresh subnet; tag the commit with epoch 0.
        let commit_epoch: u64 = 0;
        ...
        WeightCommits::<Test>::mutate(NetUidStorageIndex::from(netuid), hotkey, |maybe_commits| {
            ...
            commits.push_back((commit_hash, commit_epoch, 0, 0));
        ...
        // Advance the epoch counter into the commit's reveal epoch
        // (`commit_epoch + reveal_period`); pin the scheduler so the look-ahead
        // does not overshoot.
        ...
        // Advance the counter past the commit's reveal epoch — now too late.
        SubnetEpochIndex::<Test>::insert(netuid, commit_epoch + reveal_period + 1);
        LastEpochBlock::<Test>::insert(netuid, SubtensorModule::get_current_block_as_u64());
...
fn test_batch_reveal_weights_validate() {
    ...
    new_test_ext(0).execute_with(|| {
        ...
        // Epoch all the commits were tagged with (committed in a tight loop).
        let commit_epoch =
        ...
        // Advance the epoch counter into the commits' reveal epoch.
        SubnetEpochIndex::<Test>::insert(netuid, commit_epoch + reveal_period);
        LastEpochBlock::<Test>::insert(netuid, SubtensorModule::get_current_block_as_u64());
        PendingEpochAt::<Test>::insert(netuid, 0);
        ...
        // Test 6: CommitBlockNotInRevealRange - reveal too late (counter past reveal epoch)
        SubnetEpochIndex::<Test>::insert(netuid, commit_epoch + reveal_period + 1);
        LastEpochBlock::<Test>::insert(netuid, SubtensorModule::get_current_block_as_u64());
