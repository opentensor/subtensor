...
// SKIP_WASM_BUILD=1 RUST_LOG=debug cargo test --package pallet-subtensor --lib -- tests::coinbase::test_dynamic_function_zero_price_get_zero_tao_in --exact --show-output --nocapture
#[test]
fn test_dynamic_function_zero_price_get_zero_tao_in() {
    new_test_ext(1).execute_with(|| {
        ...
        let (tao_in, alpha_in, alpha_out): (u64, u64, u64) = SubtensorModule::get_dynamic_tao_emission( 
            netuid, tao_block_emission, tao_block_emission, alpha_block_emission
        );
        assert_eq!( tao_in, 0 ); // tao emission drops to price which is zero given no reserves.
...
fn test_dynamic_function_price_equal_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 110, 100);
        let (tao_in, alpha_in, alpha_out): (u64, u64, u64) = SubtensorModule::get_dynamic_tao_emission( 
            netuid, tao_subnet_emission, tao_block_emission, alpha_block_emission
        );
