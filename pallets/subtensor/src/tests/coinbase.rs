...
fn test_coinbase_tao_issuance_different_prices() {
    new_test_ext(1).execute_with(|| {
        let netuid1 = NetUid::from(1);
        let netuid2 = NetUid::from(2);
        let emission = TaoCurrency::from(100_000_000);
        add_network(netuid1, 1, 0);
        add_network(netuid2, 1, 0);
        ...
        assert_eq!(
            ...
            emission / 3.into() + emission / 3.into()
        );
        close(TotalIssuance::<Test>::get().into(), emission.into(), 2);
        close(TotalStake::<Test>::get().into(), emission.into(), 2);
    });
}
