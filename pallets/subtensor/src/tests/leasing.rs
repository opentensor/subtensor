...
#[test]
fn test_terminate_lease_repatriates_residual_alpha_and_lock_state() {
    new_test_ext(1).execute_with(|| {
        let crowdloan_id = 0;
        let beneficiary = U256::from(1);
        ...
        LastColdkeyHotkeyStakeBlock::<Test>::insert(lease.coldkey, lease.hotkey, 42);
        DecayingLock::<Test>::insert(lease.coldkey, lease.netuid, false);
        ...
        assert_eq!(
            SubtensorModule::total_coldkey_alpha_on_subnet(&lease.coldkey, lease.netuid),
            residual_alpha
        );
        assert!(OwnerLock::<Test>::get(lease.netuid).is_some());
        ...
        assert!(Lock::<Test>::get((lease.coldkey, lease.netuid, lease.hotkey)).is_none());
        assert!(Lock::<Test>::get((lease.coldkey, lease.netuid, beneficiary_hotkey)).is_none());
        let beneficiary_lock =
            Lock::<Test>::get((beneficiary, lease.netuid, beneficiary_hotkey)).unwrap();
        assert_eq!(beneficiary_lock.locked_mass, lock_amount);
        assert_eq!(
            beneficiary_lock.conviction,
            U64F64::saturating_from_num(u64::from(lock_amount))
        );

        assert!(HotkeyLock::<Test>::get(lease.netuid, lease.hotkey).is_none());
        assert!(DecayingHotkeyLock::<Test>::get(lease.netuid, lease.hotkey).is_none());
        assert!(OwnerLock::<Test>::get(lease.netuid).is_none());
        assert_eq!(
            DecayingOwnerLock::<Test>::get(lease.netuid)
                .unwrap()
                .locked_mass,
            lock_amount
        );
        ...
        assert!(LastColdkeyHotkeyStakeBlock::<Test>::get(lease.coldkey, lease.hotkey).is_none());
        assert!(SubnetUidToLeaseId::<Test>::get(lease.netuid).is_none());
    });
}

#[test]
fn test_terminate_lease_repatriates_below_minimum_residual_alpha() {
    new_test_ext(1).execute_with(|| {
        let crowdloan_id = 0;
        let beneficiary = U256::from(1);
        let deposit = 10_000_000_000;
        ...
        let dust_alpha = AlphaBalance::from(1_u64);
        SubtensorModule::increase_stake_for_hotkey_and_coldkey_on_subnet(
            &lease.hotkey,
            &lease.coldkey,
            lease.netuid,
...
#[test]
fn test_terminate_lease_repatriates_lock_without_residual_alpha() {
    new_test_ext(1).execute_with(|| {
        let crowdloan_id = 0;
        let beneficiary = U256::from(1);
        let deposit = 10_000_000_000;
        ...
        assert!(Lock::<Test>::get((lease.coldkey, lease.netuid, lease.hotkey)).is_some());
        ...
        assert!(Lock::<Test>::get((lease.coldkey, lease.netuid, beneficiary_hotkey)).is_none());
        assert_eq!(
            Lock::<Test>::get((beneficiary, lease.netuid, beneficiary_hotkey))
                .unwrap()
                .locked_mass,
            lock_amount
        );
...
#[test]
fn test_terminate_lease_merges_partial_residual_lock_with_existing_beneficiary_lock() {
    new_test_ext(1).execute_with(|| {
        let crowdloan_id = 0;
        let beneficiary = U256::from(1);
        let deposit = 10_000_000_000;
        ...
        let end_block = 500;
        let residual_alpha = AlphaBalance::from(2_000_000_u64);
        let lease_lock_amount = AlphaBalance::from(4_000_000_u64);
        let beneficiary_lock_amount = AlphaBalance::from(1_000_000_u64);
        let (lease_id, lease) = setup_leased_network(
            beneficiary,
            Percent::from_percent(30),
            Some(end_block),
        ...
        SubtensorModule::decrease_stake_for_hotkey_and_coldkey_on_subnet(
            ...
            &lease.coldkey,
            lease.netuid,
            lease_lock_amount.saturating_sub(residual_alpha),
        );
        ...
        assert_eq!(
            Lock::<Test>::get((beneficiary, lease.netuid, beneficiary_hotkey))
                .unwrap()
                .locked_mass,
            beneficiary_lock_amount.saturating_add(lease_lock_amount)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            beneficiary_lock_amount.saturating_add(residual_alpha)
        );
        assert_eq!(
            SubtensorModule::total_coldkey_alpha_on_subnet(&lease.coldkey, lease.netuid),
            AlphaBalance::ZERO
...
#[test]
fn test_terminate_lease_rolls_back_if_repatriated_lock_conflicts() {
    new_test_ext(1).execute_with(|| {
        let crowdloan_id = 0;
        let beneficiary = U256::from(1);
        let deposit = 10_000_000_000;
        ...
        let beneficiary_hotkey = U256::from(3);
        let conflicting_hotkey = U256::from(4);
        assert_ok!(SubtensorModule::create_account_if_non_existent(
            &beneficiary,
            &beneficiary_hotkey
        ));
