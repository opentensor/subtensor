...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_validator_exclude_quantile(netuid: u16) -> u16 {
        ValidatorExcludeQuantile::<T>::get(netuid)
    }
    ...
    pub fn do_sudo_set_validator_exclude_quantile(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        ensure_root(origin)?;
            ...
            validator_exclude_quantile <= u16::MAX,
            Error::<T>::StorageValueOutOfRange
        ); // The quantile must be between 0 and u16::MAX => 0% and 100%
        Self::set_validator_exclude_quantile(netuid, validator_exclude_quantile);
        log::info!(
            "ValidatorExcludeQuantileSet( netuid: {:?} validator_exclude_quantile: {:?} ) ",
            netuid,
            validator_exclude_quantile
        );
        Self::deposit_event(Event::ValidatorExcludeQuantileSet(
            netuid,
            validator_exclude_quantile,
        ));
        Ok(())
    ...
    pub fn get_validator_logits_divergence(netuid: u16) -> u16 {
        ValidatorLogitsDivergence::<T>::get(netuid)
    }
    pub fn set_validator_logits_divergence(netuid: u16, validator_logits_divergence: u16) {
        ValidatorLogitsDivergence::<T>::insert(netuid, validator_logits_divergence);
    }
    pub fn do_sudo_set_validator_logits_divergence(
        origin: T::RuntimeOrigin,
        netuid: u16,
        validator_logits_divergence: u16,
    ) -> DispatchResult {
        ensure_root(origin)?;
        ensure!(
            Self::if_subnet_exist(netuid),
        ...
        log::info!(
            "ValidatorLogitsDivergenceSet( netuid: {:?} validator_logits_divergence: {:?} ) ",
            netuid,
            validator_logits_divergence
        );
        Self::deposit_event(Event::ValidatorLogitsDivergenceSet(
            netuid,
            validator_logits_divergence,
        ));
        Ok(())
    ...
    pub fn get_synergy_scaling_law_power(netuid: u16) -> u16 {
        SynergyScalingLawPower::<T>::get(netuid)
    }
    pub fn set_synergy_scaling_law_power(netuid: u16, synergy_scaling_law_power: u16) {
        SynergyScalingLawPower::<T>::insert(netuid, synergy_scaling_law_power);
    }
    pub fn do_sudo_set_synergy_scaling_law_power(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        ensure_root(origin)?;
            ...
            synergy_scaling_law_power <= 100,
            Error::<T>::StorageValueOutOfRange
        ); // The synergy scaling law power must be between 0 and 100 => 0% and 100%
        Self::set_synergy_scaling_law_power(netuid, synergy_scaling_law_power);
        log::info!(
            "SynergyScalingLawPowerSet( netuid: {:?} synergy_scaling_law_power: {:?} ) ",
            netuid,
            synergy_scaling_law_power
        );
        Self::deposit_event(Event::SynergyScalingLawPowerSet(
            netuid,
            synergy_scaling_law_power,
        ));
        Ok(())
    ...
    pub fn set_validator_epochs_per_reset(netuid: u16, validator_epochs_per_reset: u16) {
        ValidatorEpochsPerReset::<T>::insert(netuid, validator_epochs_per_reset);
    }
    pub fn do_sudo_set_validator_epochs_per_reset(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        log::info!(
            "ValidatorEpochPerResetSet( netuid: {:?} validator_epochs_per_reset: {:?} ) ",
            netuid,
            validator_epochs_per_reset
        );
        Self::deposit_event(Event::ValidatorEpochPerResetSet(
            netuid,
            validator_epochs_per_reset,
        ));
        Ok(())
    ...
    pub fn set_validator_sequence_length(netuid: u16, validator_sequence_length: u16) {
        ValidatorSequenceLength::<T>::insert(netuid, validator_sequence_length);
    }
    pub fn do_sudo_set_validator_sequence_length(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        ValidatorSequenceLength::<T>::insert(netuid, validator_sequence_length);
        log::info!(
            "ValidatorSequenceLengthSet( netuid: {:?} validator_sequence_length: {:?} ) ",
            netuid,
            validator_sequence_length
        );
        Self::deposit_event(Event::ValidatorSequenceLengthSet(
            netuid,
            validator_sequence_length,
        ));
        Ok(())
    ...
    pub fn get_validator_epoch_length(netuid: u16) -> u16 {
        ValidatorEpochLen::<T>::get(netuid)
    }
    pub fn set_validator_epoch_length(netuid: u16, validator_epoch_length: u16) {
        ValidatorEpochLen::<T>::insert(netuid, validator_epoch_length);
    }
    pub fn do_sudo_set_validator_epoch_length(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        ValidatorEpochLen::<T>::insert(netuid, validator_epoch_length);
        log::info!(
            "ValidatorEpochLengthSet( netuid: {:?} validator_epoch_length: {:?} ) ",
            netuid,
            validator_epoch_length
        );
        Self::deposit_event(Event::ValidatorEpochLengthSet(
            netuid,
            validator_epoch_length,
        ));
        Ok(())
    ...
    pub fn set_validator_batch_size(netuid: u16, validator_batch_size: u16) {
        ValidatorBatchSize::<T>::insert(netuid, validator_batch_size);
    }
    pub fn do_sudo_set_validator_batch_size(
        origin: T::RuntimeOrigin,
        netuid: u16,
        ...
        log::info!(
            "ValidatorBatchSizeSet( netuid: {:?} validator_batch_size: {:?} ) ",
            netuid,
