...
impl<T: Config> Pallet<T> {
    ...
    pub fn run_coinbase() {
        ...
        for &netuid in subnets.iter() {
            ...
            // Skip netuid payouts for root network and when subnet registrations are off
        ...
        // Reveal weights from the n-2nd epoch.
        for netuid in subnets.iter() {
            if Self::should_run_epoch(*netuid, current_block) {
        ...
        // Apply pending childkeys of this subnet for the next epoch
        for netuid in subnets.iter() {
            if Self::should_run_epoch(*netuid, current_block) {
