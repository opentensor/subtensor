...
pub static SYMBOLS: [&[u8]; 439] = [
    ...
    b"\xD7\x98", // ט (Tet)
    b"\xD7\x99", // י (Yod)
    b"\xD7\x9A", // ך (Final Kaf)
    b"\xD7\x9B", // כ (Kaf)
    b"\xD7\x9C", // ל (Lamed)
    b"\xD7\x9D", // ם (Final Mem)
    b"\xD7\x9E", // מ (Mem)
    b"\xD7\x9F", // ן (Final Nun)
    b"\xD7\xA0", // נ (Nun)
    b"\xD7\xA1", // ס (Samekh)
    b"\xD7\xA2", // ע (Ayin)
    b"\xD7\xA3", // ף (Final Pe)
    b"\xD7\xA4", // פ (Pe)
    b"\xD7\xA5", // ץ (Final Tsadi)
    b"\xD7\xA6", // צ (Tsadi)
    b"\xD7\xA7", // ק (Qof)
    b"\xD7\xA8", // ר (Resh)
    b"\xD7\xA9", // ש (Shin)
...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_symbol_for_subnet(netuid: NetUid) -> Vec<u8> {
        match u16::from(netuid) {
            ...
            // Runic Alphabet
            81 => b"\xE1\x9A\xA0".to_vec(), // ᚠ (Fehu, wealth, 81)
            82 => b"\xE1\x9A\xA2".to_vec(), // ᚢ (Uruz, strength, 82)
            83 => b"\xE1\x9A\xA6".to_vec(), // ᚦ (Thurisaz, giant, 83)
            ...
            // Ogham Alphabet
            90 => b"\xE1\x9A\x80".to_vec(), //   (Space, 90)
            91 => b"\xE1\x9A\x81".to_vec(), // ᚁ (Beith, birch, 91)
            92 => b"\xE1\x9A\x82".to_vec(), // ᚂ (Luis, rowan, 92)
            93 => b"\xE1\x9A\x83".to_vec(), // ᚃ (Fearn, alder, 93)
            94 => b"\xE1\x9A\x84".to_vec(), // ᚄ (Sail, willow, 94)
            95 => b"\xE1\x9A\x85".to_vec(), // ᚅ (Nion, ash, 95)
            96 => b"\xE1\x9A\x9B".to_vec(), // ᚛ (Forfeda, 96)

            // Georgian Alphabet (Mkhedruli)
            97 => b"\xE1\x83\x90".to_vec(),  // ა (Ani, 97)
            98 => b"\xE1\x83\x91".to_vec(),  // ბ (Bani, 98)
            99 => b"\xE1\x83\x92".to_vec(),  // გ (Gani, 99)
            100 => b"\xE1\x83\x93".to_vec(), // დ (Doni, 100)
            101 => b"\xE1\x83\x94".to_vec(), // ე (Eni, 101)
            102 => b"\xE1\x83\x95".to_vec(), // ვ (Vini, 102)

            // Armenian Alphabet
            103 => b"\xD4\xB1".to_vec(), // Ա (Ayp, 103)
            104 => b"\xD4\xB2".to_vec(), // Բ (Ben, 104)
            105 => b"\xD4\xB3".to_vec(), // Գ (Gim, 105)
            106 => b"\xD4\xB4".to_vec(), // Դ (Da, 106)
            107 => b"\xD4\xB5".to_vec(), // Ե (Ech, 107)
            108 => b"\xD4\xB6".to_vec(), // Զ (Za, 108)
            109 => b"\xD5\x9E".to_vec(), // ՞ (Question mark, 109)

            // Cyrillic Alphabet
            110 => b"\xD0\x80".to_vec(), // Ѐ (Ie with grave, 110)
            111 => b"\xD0\x81".to_vec(), // Ё (Io, 111)
            112 => b"\xD0\x82".to_vec(), // Ђ (Dje, 112)
            113 => b"\xD0\x83".to_vec(), // Ѓ (Gje, 113)
            114 => b"\xD0\x84".to_vec(), // Є (Ukrainian Ie, 114)
            115 => b"\xD0\x85".to_vec(), // Ѕ (Dze, 115)
            116 => b"\xD1\x8A".to_vec(), // Ъ (Hard sign, 116)
            ...
            // End of Sinhala Alphabet
            128 => b"\xE0\xB6\xB1".to_vec(), // ඲ (La, 128)
            129 => b"\xE0\xB6\xB2".to_vec(), // ඳ (Va, 129)
            130 => b"\xE0\xB6\xB3".to_vec(), // ප (Sha, 130)
            131 => b"\xE0\xB6\xB4".to_vec(), // ඵ (Ssa, 131)
            132 => b"\xE0\xB6\xB5".to_vec(), // බ (Sa, 132)
            133 => b"\xE0\xB6\xB6".to_vec(), // භ (Ha, 133)
