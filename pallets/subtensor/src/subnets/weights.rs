...
impl<T: Config> Pallet<T> {
    ...
    /// Same preconditions as [`Self::internal_commit_weights`], without persisting a new commit.
    /// Used by [`crate::extensions::SubtensorTransactionExtension`] for `Pays::No` commit calls.
    pub fn validate_hash_commit_weights_prerequisites(
        who: &T::AccountId,
        netuid: NetUid,
        mecid: MechId,
    ) -> Result<(), Error<T>> {
        ...
        let mut commits: VecDeque<(H256, u64, u64, u64)> =
            WeightCommits::<T>::get(netuid_index, who).unwrap_or_default();
    ...
    /// Same preconditions as [`Self::internal_commit_timelocked_weights`], without persisting.
    pub fn validate_timelocked_commit_weights_prerequisites(
        who: &T::AccountId,
        netuid: NetUid,
        mecid: MechId,
        commit_reveal_version: u16,
        ...
        let commits = TimelockedWeightCommits::<T>::get(netuid_index, cur_epoch);
        let unrevealed_commits_for_who = commits
            .iter()
            .filter(|(account, _, _, _)| account == who)
            .count();
