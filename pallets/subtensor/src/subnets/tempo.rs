...
impl<T: Config> Pallet<T> {
    ...
    pub fn adjust_tempos() {
        ...
        let subnets = Self::get_all_subnet_netuids().into_iter().filter(|&netuid| netuid != 0).collect::<Vec<u16>>();
        ...
        let subnet_tempos = Self::calculate_tempos(
            k, 
            subnet_tao, 
            min_tempo, 
            max_tempo
    ...
    pub fn calculate_tempos(
        ...
        log::debug!("Checking if min_tempo >= max_tempo: min_tempo = {}, max_tempo = {}", min_tempo, max_tempo);
        if min_tempo >= max_tempo {
            log::debug!("min_tempo >= max_tempo, returning vec of max_tempo with length {}", tao.len());
        ...
        let total_tao: I96F32 = I96F32::from_num(tao.iter().copied().fold(0u64, |acc, x| acc.saturating_add(x)));
        ...
        if total_tao == I96F32::from_num(0) {
            ...
            let average_tempo = I96F32::from_num(k).clamp(I96F32::from_num(min_tempo), I96F32::from_num(max_tempo)).to_num::<u16>();
        ...
        let norm_tao: Vec<I96F32> = tao.iter().map(|&x| {
        ...
        let tempos: Vec<I96F32> = norm_tao.into_iter().map(|value| {
            if value > 0 {
                let tempo = (I96F32::from_num(k) / value).clamp(I96F32::from_num(min_tempo), I96F32::from_num(max_tempo));
