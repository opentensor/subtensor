...
impl<T: Config> Pallet<T> 
{
    ...
    pub fn epoch_dense(netuid: u16, rao_emission: u64) -> Vec<(T::AccountId, u64, u64)> 
    {
        ...
        let outdated:               Vec<Vec<bool>>  = last_update.iter().map(|updated| 
            block_at_registration.iter().map(|registered|
                updated <= registered)
            .collect()
        ).collect();
        ...
        let validator_forbids:      Vec<bool>   = validator_permits.iter().map(|&b| !b).collect(); // Logical negation of validator_permits.
        ...
        let consensus: Vec<I32F32>;
        {
            ...
            consensus           = weighted_median_col(&active_stake, &weights, kappa);
    ...
    pub fn epoch( netuid: u16, rao_emission: u64 ) -> Vec<(T::AccountId, u64, u64)> {
        ...
        let stake: Vec<I32F32> = vec_fixed64_to_fixed32( stake_64 ); // range: I32F32(0, 1)
        log::trace!( "S: {:?}", &stake );
        ...
        let consensus:  Vec<I32F32>;
        {
            ...
            consensus               = weighted_median_col_sparse(&active_stake, &weights, n, kappa);
        ...
        // Access network bonds.
        let mut bonds: Vec<Vec<(u16, I32F32)>>;
        {
            bonds = Self::get_bonds_sparse(netuid);
            // log::trace!( "B: {:?}", &bonds );
        ...
        { 
            bonds_delta = row_hadamard_sparse(&weights, &active_stake); // ΔB = W◦S (outdated W masked)
