...
impl<T: Config> Pallet<T> {
    ...
    pub fn run_coinbase(block_number: u64) {
        ...
        for netuid in netuids.iter() {
            ...
            if *netuid == Self::get_root_netuid() { continue; }
            if Self::is_subnet_dynamic(*netuid) {
                // Append the DTAO subnet price.
                let price = Self::get_tao_per_alpha_price(*netuid);
                dtao_subnets.push((*netuid, price));
                dtao_total_prices += price;
        ...
        if dtao_total_prices != I64F64::from_num(0.0) && dtao_subnets.len() != 0 {
            for (netuid, subnet_price) in dtao_subnets.iter() {
                // Compute the subnet normalized price.
                let normalized_subnet_price: I64F64 = subnet_price / I64F64::from_num(dtao_total_prices);
                // Compute the emission based on the price normalized.
                let subnet_tao_emission: I64F64 = dtao_emission * normalized_subnet_price;
