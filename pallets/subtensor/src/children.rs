...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_set_child_singular(
        ...
        for (_, old_child) in old_children.clone().iter() {
            ...
            let my_old_child_parents: Vec<(u64, T::AccountId)> = ParentKeys::<T>::get( old_child.clone(), netuid );
        ...
        for (proportion, new_child) in new_children.clone().iter() {
            ...
            let mut new_child_previous_parents: Vec<(u64, T::AccountId)> = ParentKeys::<T>::get( new_child.clone(), netuid );
        ...
        Self::deposit_event(Event::SetChildSingular(hotkey.clone(), child, netuid, proportion));
