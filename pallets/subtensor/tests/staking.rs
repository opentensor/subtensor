...
fn test_add_stake_dispatch_info_ok() {
    new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::SubtensorModule(SubtensorModuleCall::add_stake {
            hotkey,
            amount_staked,
        });
        assert_eq!(
...
fn test_remove_stake_dispatch_info_ok() {
    new_test_ext().execute_with(|| {
        ...
        let call = RuntimeCall::SubtensorModule(SubtensorModuleCall::remove_stake {
            hotkey,
            amount_unstaked,
        });
        assert_eq!(
...
fn test_remove_balance_from_coldkey_account_ok() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            SubtensorModule::remove_balance_from_coldkey_account(&coldkey_account_id, ammount - 1);
