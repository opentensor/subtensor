...
fn test_do_move_success() {
    new_test_ext(1).execute_with(|| {
        ...
        // Perform the move
        assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), origin_hotkey, destination_hotkey, netuid, netuid));

        // Check that the stake has been moved
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, netuid), stake_amount);
    });
}
...
fn test_do_move_different_subnets() {
    new_test_ext(1).execute_with(|| {
        ...
        // Perform the move
        assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), origin_hotkey, destination_hotkey, origin_netuid, destination_netuid));

        // Check that the stake has been moved
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, origin_netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, destination_netuid), stake_amount);
    });
}
...
fn test_do_move_locked_funds() {
    new_test_ext(1).execute_with(|| {
        ...
        SubtensorModule::create_account_if_non_existent(&coldkey, &destination_hotkey);
        SubtensorModule::stake_into_subnet(&origin_hotkey, &coldkey, origin_netuid, stake_amount);
        assert_ok!(SubtensorModule::do_lock(RuntimeOrigin::signed(coldkey), origin_hotkey, origin_netuid, 100, lock_amount));

        // Attempt to move locked funds
        assert_noop!(
            SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), origin_hotkey, destination_hotkey, origin_netuid, destination_netuid),
            Error::<Test>::NotEnoughStakeToWithdraw
        );

        // Check that the stake and lock remain unchanged
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, origin_netuid), stake_amount);
        assert_eq!(SubtensorModule::get_locked_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, origin_netuid), lock_amount);
    });
}
...
fn test_do_move_nonexistent_subnet() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_noop!(
            SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), origin_hotkey, destination_hotkey, origin_netuid, nonexistent_netuid),
...
fn test_do_move_nonexistent_origin_hotkey() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 0, 0 );
        assert_noop!(
            SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), nonexistent_origin_hotkey, destination_hotkey, netuid, netuid),
            Error::<Test>::HotKeyAccountNotExists
        );

        // Check that no stake was moved
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&nonexistent_origin_hotkey, &coldkey, netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, netuid), 0);
    });
}
...
fn test_do_move_nonexistent_destination_hotkey() {
    new_test_ext(1).execute_with(|| {
        ...
        add_network(netuid, 0, 0 );
        assert_noop!(
            SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), origin_hotkey, nonexistent_destination_hotkey, netuid, netuid),
            Error::<Test>::HotKeyAccountNotExists
        );

        // Check that the stake was moved successfully
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, netuid), 1000);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&nonexistent_destination_hotkey, &coldkey, netuid), 0);
    });
}
...
fn test_do_move_partial_stake() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, netuid), total_stake);
    });
}
...
fn test_do_move_multiple_times() {
    new_test_ext(1).execute_with(|| {
        ...
        for _ in 0..3 {
            assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), hotkey1, hotkey2, netuid, netuid));
            assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), hotkey2, hotkey1, netuid, netuid));
        }

        // Check final stake distribution
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&hotkey1, &coldkey, netuid), initial_stake);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&hotkey2, &coldkey, netuid), 0);
    });
}
...
fn test_do_move_with_locks() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(SubtensorModule::do_lock(RuntimeOrigin::signed(coldkey), origin_hotkey, netuid, 100, lock_amount));
...
fn test_do_move_wrong_origin() {
    new_test_ext(1).execute_with(|| {
        ...
        SubtensorModule::create_account_if_non_existent(&coldkey, &origin_hotkey);
        SubtensorModule::create_account_if_non_existent(&coldkey, &destination_hotkey);
        assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(wrong_coldkey), origin_hotkey, destination_hotkey, netuid, netuid));

        // Check that no stake was moved
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, netuid), stake_amount);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, netuid), 0);
    });
}
...
fn test_do_move_same_hotkey() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_ok!(SubtensorModule::do_move(RuntimeOrigin::signed(coldkey), hotkey, hotkey, netuid, netuid));

        // Check that stake remains unchanged
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&hotkey, &coldkey, netuid), stake_amount);
    });
}
...
fn test_do_move_event_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        System::assert_last_event(Event::StakeMoved(coldkey, origin_hotkey, netuid, destination_hotkey, netuid).into());
    });
}
...
fn test_do_move_storage_updates() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, origin_netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, destination_netuid), stake_amount);
        assert_eq!(SubtensorModule::get_locked_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, origin_netuid), 0);
        assert_eq!(SubtensorModule::get_locked_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, destination_netuid), 0);
    });
}
...
fn test_do_move_max_values() {
    new_test_ext(1).execute_with(|| {
        ...
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&origin_hotkey, &coldkey, netuid), 0);
        assert_eq!(SubtensorModule::get_stake_for_hotkey_and_coldkey_on_subnet(&destination_hotkey, &coldkey, netuid), max_stake);
    });
}
