...
fn test_loaded_emission() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(SubtensorModule::do_set_emission_values(
...
#[test]
fn test_emission_calculation_one_tenth() {
    new_test_ext().execute_with(|| {
        // Create network 1
        let netuid1: u16 = 1;
        let tempo1: u16 = 13;
...
#[test]
fn test_emission_calculation_with_empty_network() {
    new_test_ext().execute_with(|| {
        // Create network 1
        let netuid1: u16 = 1;
        let tempo1: u16 = 13;
        ...
        for i in 12u16..17u16 {
            ...
            SubtensorModule::create_account_if_non_existent( &key, &key );
            SubtensorModule::append_neuron(netuid3, &key, 0);

            SubtensorModule::increase_stake_on_coldkey_hotkey_account( &key, &key, stake );
            SubtensorModule::set_validator_permit_for_uid(netuid3, i, true);
        }
