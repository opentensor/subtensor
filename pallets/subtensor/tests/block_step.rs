...
fn test_loaded_emission() {
    new_test_ext().execute_with(|| {
        ...
        Subtensor::set_emission_values( &netuids, emission);
        for i in 0..n {
            Subtensor::append_neuron(netuid, &U256::from(i), 0);
        }
        assert!(!Subtensor::has_loaded_emission_tuples(netuid));

        // Try loading at block 0
        let block: u64 = 0;
        assert_eq!(
            Subtensor::blocks_until_next_epoch(netuid, tempo, block),
            8
        );
        Subtensor::generate_emission(block);
        assert!(!Subtensor::has_loaded_emission_tuples(netuid));
        ...
        assert!(Subtensor::has_loaded_emission_tuples(netuid));
        assert_eq!(
            Subtensor::get_loaded_emission_tuples(netuid).len(),
            n as usize
        );
        ...
        let block: u64 = 8;
        Subtensor::drain_emission(block);
        assert!(!Subtensor::has_loaded_emission_tuples(netuid));

        // Generate more emission.
        Subtensor::generate_emission(8);
        assert_eq!(
            Subtensor::get_loaded_emission_tuples(netuid).len(),
            n as usize
        );
        ...
        for block in 9..19 {
            ...
            if Subtensor::has_loaded_emission_tuples(netuid) {
                n_remaining = Subtensor::get_loaded_emission_tuples(netuid).len();
                n_to_drain = Subtensor::tuples_to_drain_this_block(
                    netuid,
                    tempo,
                    block,
                    Subtensor::get_loaded_emission_tuples(netuid).len(),
            ...
            Subtensor::drain_emission(block); // drain it with 9 more blocks to go
            if Subtensor::has_loaded_emission_tuples(netuid) {
                assert_eq!(
                    Subtensor::get_loaded_emission_tuples(netuid).len(),
...
fn test_tuples_to_drain_this_block() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 1, 0, 10), 10); // drain all epoch block.
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 0, 0, 10), 10); // drain all no tempo.
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 10, 0, 10), 2); // drain 10 / ( 10 / 2 ) = 2
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 20, 0, 10), 1); // drain 10 / ( 20 / 2 ) = 1
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 10, 0, 20), 5); // drain 20 / ( 9 / 2 ) = 5
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 20, 0, 0), 0); // nothing to drain.
        assert_eq!(Subtensor::tuples_to_drain_this_block(0, 10, 1, 20), 5); // drain 19 / ( 10 / 2 ) = 4
        assert_eq!(
            Subtensor::tuples_to_drain_this_block(0, 10, 10, 20),
            4
        ); // drain 19 / ( 10 / 2 ) = 4
        assert_eq!(
            Subtensor::tuples_to_drain_this_block(0, 10, 15, 20),
            10
        ); // drain 19 / ( 10 / 2 ) = 4
        assert_eq!(
            Subtensor::tuples_to_drain_this_block(0, 10, 19, 20),
            20
        ); // drain 19 / ( 10 / 2 ) = 4
        assert_eq!(
            Subtensor::tuples_to_drain_this_block(0, 10, 20, 20),
            20
        ); // drain 19 / ( 10 / 2 ) = 4
        for i in 0..10 {
            for j in 0..10 {
                for k in 0..10 {
                    for l in 0..10 {
                        assert!(Subtensor::tuples_to_drain_this_block(i, j, k, l) <= 10);
                    }
                }
            }
        }
...
fn test_blocks_until_epoch() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::blocks_until_next_epoch(0, 0, 0), 1000);

        // Check tempo = 1 block = * netuid = *
        assert_eq!(Subtensor::blocks_until_next_epoch(0, 1, 0), 0);
        assert_eq!(Subtensor::blocks_until_next_epoch(1, 1, 0), 1);
        assert_eq!(Subtensor::blocks_until_next_epoch(0, 1, 1), 1);
        assert_eq!(Subtensor::blocks_until_next_epoch(1, 1, 1), 0);
        assert_eq!(Subtensor::blocks_until_next_epoch(0, 1, 2), 0);
        assert_eq!(Subtensor::blocks_until_next_epoch(1, 1, 2), 1);
        for i in 0..100 {
            if i % 2 == 0 {
                assert_eq!(Subtensor::blocks_until_next_epoch(0, 1, i), 0);
                assert_eq!(Subtensor::blocks_until_next_epoch(1, 1, i), 1);
            } else {
                assert_eq!(Subtensor::blocks_until_next_epoch(0, 1, i), 1);
                assert_eq!(Subtensor::blocks_until_next_epoch(1, 1, i), 0);
            }
        }
...
fn test_burn_adjustment() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!( Subtensor::get_adjustment_interval(netuid), adjustment_interval ); // Sanity check the adjustment interval.
...
fn test_burn_adjustment_with_moving_average() {
    new_test_ext().execute_with(|| {
        ...
        // Set alpha here.
        Subtensor::set_adjustment_alpha(netuid, u64::MAX / 2);
        ...
        assert_eq!(Subtensor::get_burn_as_u64(netuid), 1250);
    });
}
...
fn test_burn_adjustment_case_a() {
    ...
    new_test_ext().execute_with(|| {
        ...
        add_network(netuid, tempo, 0);
        Subtensor::set_burn(netuid, burn_cost);
        Subtensor::set_difficulty(netuid, start_diff);
        Subtensor::set_min_difficulty(netuid, start_diff);
        Subtensor::set_adjustment_interval(netuid, adjustment_interval);
        Subtensor::set_target_registrations_per_interval(
            netuid,
            target_registrations_per_interval,
        ...
        let result0 = Subtensor::register(
            <<Test as Config>::RuntimeOrigin>::signed(hotkey_account_id_2),
            netuid,
            curr_block_num,
            nonce0,
        ...
        let result1 = Subtensor::register(
        ...
        let adjusted_burn = Subtensor::get_burn_as_u64(netuid);
        assert_eq!(adjusted_burn, burn_cost);

        let adjusted_diff = Subtensor::get_difficulty_as_u64(netuid);
