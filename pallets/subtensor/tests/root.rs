...
fn test_network_prune_results() {
    new_test_ext(1).execute_with(|| {
        ...
        // lowest emission
        assert_ok!(SubtensorModule::set_emission_values(&vec![1u16, 2u16, 3u16], vec![5u64, 4u64, 4u64]));
        assert_eq!(SubtensorModule::get_subnet_to_prune(), 2u16);

        // equal emission, creation date
        assert_ok!(SubtensorModule::set_emission_values(&vec![1u16, 2u16, 3u16], vec![5u64, 5u64, 4u64]));
        assert_eq!(SubtensorModule::get_subnet_to_prune(), 3u16);

        // equal emission, creation date
        assert_ok!(SubtensorModule::set_emission_values(&vec![1u16, 2u16, 3u16], vec![4u64, 5u64, 5u64]));
