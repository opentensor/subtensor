...
fn test_root_register_network_exist() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Subtensor::root_register(
...
fn test_root_register_normal_on_root_fails() {
    new_test_ext().execute_with(|| {
        ...
        Subtensor::set_burn(root_netuid, 0);
        Subtensor::add_balance_to_coldkey_account(&coldkey_account_id, 1);
        assert_eq!(
...
fn test_root_register_stake_based_pruning_works() {
    new_test_ext().execute_with(|| {
        ...
        Subtensor::set_max_registrations_per_block(root_netuid, 1000);
        Subtensor::set_target_registrations_per_interval(root_netuid, 1000);
        ...
        for i in 0..128 {
            ...
            assert!(!Subtensor::hotkey_is_delegate(&hot));
        }
        ...
        for i in 0..64 {
            ...
            // Check succesfull registration.
            assert!(Subtensor::get_uid_for_net_and_hotkey(root_netuid, &hot).is_ok());
...
fn test_root_set_weights() {
    new_test_ext().execute_with(|| {
        ...
        log::info!("subnet limit: {:?}", Subtensor::get_max_subnets());
        log::info!("current subnet count: {:?}", Subtensor::get_num_subnets());
        ...
        for netuid in 1..n {
            ...
            assert_eq!(
                Subtensor::get_subnet_emission_value(netuid as u16),
                99_999_999
            );
        }
        step_block(2);
        ...
        for netuid in 1..n {
            log::debug!(
                ...
                netuid,
                Subtensor::get_pending_emission(netuid as u16)
            );
            assert_eq!(
                Subtensor::get_pending_emission(netuid as u16),
                199_999_998
        ...
        let step = Subtensor::blocks_until_next_epoch(10, 1000, Subtensor::get_current_block_as_u64());
        step_block(step as u16);
        assert_eq!(Subtensor::get_pending_emission(10), 0);
    });
}
...
fn test_root_set_weights_out_of_order_netuids() {
    new_test_ext().execute_with(|| {
        ...
        log::info!("netuids: {:?}", Subtensor::get_all_subnet_netuids());
        log::info!("root network count: {:?}", Subtensor::get_subnetwork_n(0));

        let subnets = Subtensor::get_all_subnet_netuids();
        // Set weights into diagonal matrix.
        ...
        step_block(step as u16);
        assert_eq!(Subtensor::get_pending_emission(9), 0);
    });
}
...
fn test_network_pruning() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Subtensor::get_subnet_emission_value(1), 246_922_263);
        assert_eq!(Subtensor::get_subnet_emission_value(2), 215_549_466);
        ...
        assert_eq!(Subtensor::get_pending_emission(3), 176_432_500);
