...
fn test_network_pruning() {
    new_test_ext().execute_with(|| {
        ...
        for i in 0..n {
            ...
            assert!(SubtensorModule::get_uid_for_net_and_hotkey((i as u16) + 1, &hot).is_ok());

            SubtensorModule::set_tempo((i as u16) + 1, 1);
            SubtensorModule::set_burn((i as u16) + 1, 0);
            assert_eq!(
                SubtensorModule::get_subnetwork_n((i as u16) + 1),
                1
            );
        }

        assert_eq!(
            SubtensorModule::get_combined_subnet_stake(),
            55_000
        );
        ...
        assert_eq!(SubtensorModule::get_subnet_emission_value(1), 18_181_818);
        assert_eq!(SubtensorModule::get_subnet_emission_value(2), 36_363_636);
        assert_eq!(SubtensorModule::get_subnet_emission_value(3), 54_545_454);
        assert_eq!(SubtensorModule::get_subnet_emission_value(4), 72_727_272);
        assert_eq!(SubtensorModule::get_subnet_emission_value(5), 90_909_090);

        step_block(1);
        //assert_eq!(SubtensorModule::get_pending_emission(0), 0); // root network gets no pending emission.
        assert_eq!(SubtensorModule::get_pending_emission(1), 18_181_818);
        assert_eq!(SubtensorModule::get_pending_emission(2), 0); // This has been drained.
        assert_eq!(SubtensorModule::get_pending_emission(3), 54_545_454);
        assert_eq!(SubtensorModule::get_pending_emission(4), 0); // This network has been drained.
        assert_eq!(SubtensorModule::get_pending_emission(5), 90_909_090);
        step_block(1);
        assert_eq!(SubtensorModule::get_combined_subnet_stake(), 1_545_509_536);
    });
}
