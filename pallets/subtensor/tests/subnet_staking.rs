...
fn test_add_subnet_stake_ok_no_emission() 
{
    new_test_ext().execute_with(|| {
        ...
        SubtensorModule::add_balance_to_coldkey_account(&coldkey_account_id, 10000 + 1);

        // Check we have zero staked before transfer
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey_account_id),
            0
        );

        // Also total stake should be zero
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 0);
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 10000);
        assert_eq!(SubtensorModule::get_combined_subnet_stake_for_coldkey(&coldkey_account_id), 10000);
    });
}
...
fn test_subnet_dividends_with_run_to_block() {
    new_test_ext().execute_with(|| {
        ...
        SubtensorModule::inc_subnet_total_stake_for_hotkey(1, &neuron_src_hotkey_id, initial_stake);

        // Check if the initial stake has arrived
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &neuron_src_hotkey_id),
            initial_stake
        );
        ...
        // Check if the stake is equal to the inital stake + transfer
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &neuron_dest_hotkey_id),
            0
        );
    });
}
...
fn test_remove_subnet_stake_ok_no_emission() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_account_id, &hotkey_account_id, amount);
...
fn test_remove_subnet_stake_amount_zero() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 0);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_account_id, &hotkey_account_id),
            0
        );
        assert_eq!(SubtensorModule::get_coldkey_balance(&coldkey_account_id), 0);
...
fn test_remove_subnet_stake_no_enough_stake() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        assert_eq!(SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_id, &hotkey_id), 0);
...
fn test_add_stake_to_hotkey_account_ok() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_id, &hotkey_id, amount);

        // The stake that is now in the account, should equal the amount
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey_id),
            amount
        );

        // The total stake should have been increased by the amount -> 0 + amount = amount
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), amount);
    });
}
...
fn test_remove_subnet_stake_from_hotkey_account() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        // Remove stake
        SubtensorModule::dec_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_id, &hotkey_id, amount);

        // The stake on the hotkey account should be 0
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey_id), 0);
...
fn test_increase_subnet_total_stake_ok() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        let increment = 10000;
        assert_eq!(SubtensorModule::get_subnet_total_stake(1), 0);
        SubtensorModule::inc_subnet_total_stake(1, increment);
        assert_eq!(SubtensorModule::get_subnet_total_stake(1), increment);
    });
}
...
fn test_decrease_subnet_total_stake_ok() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::inc_subnet_total_stake(1, initial_total_stake);
        SubtensorModule::dec_subnet_total_stake(1, decrement);

        // The total stake remaining should be the difference between the initial stake and the decrement
        assert_eq!(
            SubtensorModule::get_subnet_total_stake(1),
...
fn test_has_enough_subnet_stake_yes() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        add_network(netuid, tempo, 0);
        register_ok_neuron(netuid, hotkey_id, coldkey_id, start_nonce);
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_id, &hotkey_id, intial_amount);
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey_id),
            10000
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey_id, &hotkey_id),
            10000
        );
        assert_eq!(
            SubtensorModule::does_coldkey_hotkey_have_enough_subnet_stake(netuid, &coldkey_id, &hotkey_id, 5000),
            true
        );
    });
}
...
fn test_has_enough_subnet_stake_no() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        register_ok_neuron(netuid, hotkey_id, coldkey_id, start_nonce);
        assert_eq!(
            SubtensorModule::does_coldkey_hotkey_have_enough_subnet_stake(netuid, &coldkey_id, &hotkey_id, amount),
            false
        );
    });
}
...
fn test_subnet_non_existent_account() {
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(
            netuid,
            &U256::from(0),
            &(U256::from(0)),
            10,
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &U256::from(0), &U256::from(0)),
            10
        );
        assert_eq!(
            SubtensorModule::get_subnet_total_stake_for_coldkey(netuid, &(U256::from(0))),
            10
        );
    });
}
...
fn test_delegate_stake_division_by_zero_check() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey, 0, 1000);
    });
}
...
fn test_subnet_full_with_delegating() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey0),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey1),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey1, &hotkey0),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey1, &hotkey1),
            0
        );
        assert_ok!(SubtensorModule::add_subnet_stake(
            <<Test as Config>::RuntimeOrigin>::signed(coldkey0),
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey0), 100);
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey1), 100);
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 200);
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 0, 100);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 0, 100);
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey0), 200);
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey1), 200);
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey0), 500);
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey1), 400);
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 900);

        // Lets emit inflation through the hot and coldkeys.
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 0, 1000);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 0, 1000);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey0),
            601
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 2900); // 600 + 700 + 900 + 700 = 2900
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2, &hotkey2),
            900
        );
        assert_eq!(
            SubtensorModule::remove_subnet_stake(
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey1, &hotkey2),
            1_000
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey2),
            1_000
        );
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey2), 3_000);
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 5_500);

        // Lets emit inflation through this new key with distributed ownership.
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey2, 0, 1000);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2, &hotkey2),
            1_668
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 6_500); // before + 1_000 = 5_500 + 1_000 = 6_500
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey3),
            1000
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey1, &hotkey3),
            1000
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2, &hotkey3),
            1000
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey3, &hotkey3),
            1000
        );
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey3), 4000);
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 10_500);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey3, 0, 1000);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey3),
            1000
        );
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 11_500); // before + 1_000 = 10_500 + 1_000 = 11_500
    });
}
...
fn test_full_with_delegating_some_servers() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 200, 1_000); // 1_200 total emission.
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 123, 2_000); // 2_123 total emission.
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey0),
            801
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey0), 1_700); // initial + server emission + validator emission = 799 + 899 = 1_698
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey1), 2_523); // 400 + 2_123
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 4_223); // 1_700 + 2_523 = 4_223
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 350, 0);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 150, 0);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0, &hotkey0),
            1_151
        ); // + 350 = 1_151
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 4_723); // 4_223 + 500 = 4_823
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 5_623); // 4_723 + 900 = 5_623
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey2), 3_000);
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 7_723); // 5_623 + (1_000 + 1_000 + 100) = 7_723
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey2, 100, 1000);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2, &hotkey2),
            1_768
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 8_823); // 7_723 + 1_100 = 8_823
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey2, 123, 0);
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2, &hotkey2),
            1_891
        ); // 1_768 + 123 = 1_891
        ...
        assert_eq!(SubtensorModule::get_subnet_total_stake(netuid), 8_946); // 8_823 + 123 = 8_946
    });
}
...
fn test_subnet_full_block_emission_occurs() {
    new_test_ext().execute_with(|| {
        ...
        // Emit inflation through non delegates.
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 0, 111);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 0, 234);
        // Verify the full emission occurs.

        let total_stake = SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey0)
                         + SubtensorModule::get_subnet_total_stake_for_hotkey(netuid, &hotkey1);
        ...
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey0, 0, 12_948);
        SubtensorModule::emit_inflation_through_hotkey_account(netuid, &hotkey1, 0, 1_874);
...
fn test_unstake_all_coldkeys_from_hotkey_account() 
{
    new_test_ext().execute_with(|| 
    {
        ...
        //Add some stake that can be removed
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0_id, &hotkey_id, amount);
        SubtensorModule::inc_subnet_stake_for_coldkey_hotkey(
            netuid,
            &coldkey1_id,
            &hotkey_id,
        ...
        SubtensorModule::remove_all_subnet_stake_for_hotkey(netuid, &hotkey_id);
        ...
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey0_id, &hotkey_id),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey1_id, &hotkey_id),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey2_id, &hotkey_id),
            0
        );
        assert_eq!(
            SubtensorModule::get_subnet_stake_for_coldkey_hotkey(netuid, &coldkey3_id, &hotkey_id),
            0
        );
