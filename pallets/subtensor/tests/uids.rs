...
fn test_replace_neuron() {
	new_test_ext().execute_with(|| {
        ...
        assert!(SubtensorModule::get_uid_for_net_and_hotkey(netuid, &new_hotkey_account_id).is_ok());
        assert!(SubtensorModule::is_hotkey_registered_on_any_network(&new_hotkey_account_id) );
...
fn test_replace_neuron_multiple_subnets_unstake_all() {
	new_test_ext().execute_with(|| {
        ...
        SubtensorModule::increase_stake_on_coldkey_hotkey_account(&coldkey_account_id, &hotkey_account_id, stake_amount);
        SubtensorModule::increase_stake_on_coldkey_hotkey_account(&coldkey_account1_id, &hotkey_account_id, stake_amount + 1);
        SubtensorModule::increase_stake_on_coldkey_hotkey_account(&coldkey_account2_id, &hotkey_account_id, stake_amount + 2);

        // Check stake on neuron
        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account_id, &hotkey_account_id), stake_amount);
        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account1_id, &hotkey_account_id), stake_amount + 1);
        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account2_id, &hotkey_account_id), stake_amount + 2);

        // Check total stake on neuron
        assert_eq!(SubtensorModule::get_total_stake_for_hotkey(&hotkey_account_id), (stake_amount * 3) + (1 + 2));
        ...
        SubtensorModule::replace_neuron(netuid1, neuron_uid.unwrap(), &new_hotkey_account_id, block_number);

        // The neuron should be unregistered now.
        assert!( !SubtensorModule::is_hotkey_registered_on_any_network(&hotkey_account_id) );

        // Check the stake is now on the free balance of the coldkey accounts.
        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account_id, &hotkey_account_id), 0);
        assert_eq!(Balances::free_balance(&coldkey_account_id), stake_amount);

        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account1_id, &hotkey_account_id), 0);
        assert_eq!(Balances::free_balance(&coldkey_account1_id), stake_amount + 1);

        assert_eq!(SubtensorModule::get_stake_for_coldkey_and_hotkey(&coldkey_account2_id, &hotkey_account_id), 0);
        assert_eq!(Balances::free_balance(&coldkey_account2_id), stake_amount + 2);
