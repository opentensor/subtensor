...
fn test_migration_fix_total_stake_maps() {
    new_test_ext().execute_with(|| {
        ...
        Subtensor::increase_stake_on_coldkey_hotkey_account(&ck1, &hk1, 100);
        total_stake_amount += 100;

        Subtensor::increase_stake_on_coldkey_hotkey_account(&ck2, &hk1, 10_101);
        total_stake_amount += 10_101;

        Subtensor::increase_stake_on_coldkey_hotkey_account(&ck3, &hk2, 100_000_000);
        total_stake_amount += 100_000_000;

        Subtensor::increase_stake_on_coldkey_hotkey_account(&ck1, &hk2, 1_123_000_000);
        total_stake_amount += 1_123_000_000;

        // Check that the total stake is correct
        assert_eq!(Subtensor::get_total_stake(), total_stake_amount);

        // Check that the total coldkey stake is correct
        assert_eq!(
            Subtensor::get_total_stake_for_coldkey(&ck1),
            100 + 1_123_000_000
        );
        assert_eq!(Subtensor::get_total_stake_for_coldkey(&ck2), 10_101);
        assert_eq!(
            Subtensor::get_total_stake_for_coldkey(&ck3),
            100_000_000
        );

        // Check that the total hotkey stake is correct
        assert_eq!(
            Subtensor::get_total_stake_for_hotkey(&hk1),
            100 + 10_101
        );
        assert_eq!(
            Subtensor::get_total_stake_for_hotkey(&hk2),
...
fn test_migration_transfer_nets_to_foundation() {
    new_test_ext().execute_with(|| {
        ...
        log::info!("{:?}", Subtensor::get_subnet_owner(1));
        //assert_eq!(Subtensor::<T>::get_subnet_owner(1), );
...
fn test_migration_delete_subnet_3() {
    new_test_ext().execute_with(|| {
        // Create subnet 3
        add_network(3, 1, 0);
        assert_eq!(Subtensor::if_subnet_exist(3), true);

        // Run the migration to transfer ownership
        pallet_subtensor::migration::migrate_delete_subnet_3::<Test>();

        assert_eq!(Subtensor::if_subnet_exist(3), false);
    })
}
...
fn test_migration_delete_subnet_21() {
    new_test_ext().execute_with(|| {
        // Create subnet 21
        add_network(21, 1, 0);
        assert_eq!(Subtensor::if_subnet_exist(21), true);

        // Run the migration to transfer ownership
        pallet_subtensor::migration::migrate_delete_subnet_21::<Test>();

        assert_eq!(Subtensor::if_subnet_exist(21), false);
    })
}