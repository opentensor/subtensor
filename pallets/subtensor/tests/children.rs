...
fn test_childkey_single_parent_emission() {
    new_test_ext(1).execute_with(|| {
        ...
        let parent_alpha_before: u64 =
            pallet_subtensor::Alpha::<Test>::get((parent_hotkey, netuid, parent_coldkey));
        let child_alpha_before: u64 =
            pallet_subtensor::Alpha::<Test>::get((child_hotkey, netuid, child_coldkey));
...
fn test_childkey_set_weights_single_parent() {
    new_test_ext(1).execute_with(|| {
        ...
        let current_stake = SubtensorModule::get_stake_for_hotkey_on_subnet(&child, netuid);
        SubtensorModule::set_weights_min_stake(current_stake * 2);
...
fn test_set_weights_no_parent() {
    ...
    new_test_ext(1).execute_with(|| {
        ...
        // Set the min stake very high
        let current_stake = SubtensorModule::get_stake_for_hotkey_on_subnet(&hotkey, netuid);
        SubtensorModule::set_weights_min_stake(current_stake * 2);
