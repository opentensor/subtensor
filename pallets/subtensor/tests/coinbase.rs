...
fn test_coinbase_basic() {
    new_test_ext(1).execute_with(|| {
        ...
        let alpha_before = pallet_subtensor::StakedAlpha::<Test>::get((hotkey, coldkey, netuid));
        ...
        let alpha_after = pallet_subtensor::StakedAlpha::<Test>::get((hotkey, coldkey, netuid));
...
fn test_distribute_owner_cut_basic() {
    new_test_ext(1).execute_with(|| {
        ...
        // Verify distribution
        assert_eq!(StakedAlpha::<Test>::get((hotkey, coldkey, netuid)), 1000);
    });
}
...
fn test_distribute_owner_cut_no_owner_hotkey() {
    new_test_ext(1).execute_with(|| {
        ...
        // Verify distribution
        assert_eq!(StakedAlpha::<Test>::get((coldkey, coldkey, netuid)), 1000);
    });
}
...
fn test_distribute_owner_cut_is_actually_used() {
    new_test_ext(1).execute_with(|| {
        ...
        assert!(StakedAlpha::<Test>::get((subnet_owner_hotkey, subnet_owner_coldkey, netuid)) == 0);
